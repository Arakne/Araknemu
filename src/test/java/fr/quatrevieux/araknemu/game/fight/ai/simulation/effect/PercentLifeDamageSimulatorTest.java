...
class PercentLifeDamageSimulatorTest extends FightBaseCase {
    private Fight fight;
    private PlayerFighter fighter;
    private Fighter target;
    private FighterAI ai;
    private PercentLifeDamageSimulator simulator;
    ...
    public void setUp() throws Exception {
        ...
        simulator = new PercentLifeDamageSimulator(container.get(Simulator.class), Element.EARTH);
    }
    ...
    void simulateWithResistance() {
        ...
        simulator = new PercentLifeDamageSimulator(container.get(Simulator.class), Element.WATER);
        assertEquals(-19, simulate().enemiesLife());
    }
    ...
    void simulatePoisonShouldIgnoreArmor() {
        ...
        assertEquals(-43.5, simulation.enemiesLife());
    }
    ...
    void simulateBuff() {
        ...
        simulator.simulate(simulation, ai, scope.effects().get(0));
        assertEquals(-108.75, simulation.enemiesLife());

        Mockito.when(effect.duration()).thenReturn(20);
        simulation = new CastSimulation(spell, fighter, target.cell());
        scope = makeCastScope(fighter, spell, effect, target.cell());
        simulator.simulate(simulation, ai, scope.effects().get(0));
        assertEquals(-217.5, simulation.enemiesLife());
        ...
        simulation = new CastSimulation(spell, fighter, target.cell());
        simulator.simulate(simulation, ai, scope.effects().get(0));
        assertEquals(-120, simulation.enemiesLife());
    }
