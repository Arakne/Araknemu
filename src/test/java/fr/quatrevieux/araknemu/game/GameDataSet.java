...
public class GameDataSet extends TestingDataSet {
    ...
    public GameDataSet pushMap(int id, String date, int width, int height, String key, String mapData, String places, Geoposition geoposition, int subAreaId) throws SQLException, ContainerException {
        Repository<MapTemplate> repository = repository(MapTemplate.class);

        if (repository.has(new MapTemplate(id, null, null, null, null, null, geoposition, subAreaId))) {
            return this;
        }

        connection.prepare(
            "INSERT INTO maps (id, date, width, height, key, mapData, places, MAP_X, MAP_Y, SUBAREA_ID) VALUES(?,?,?,?,?,?,?,?,?,?)",
            statement -> {
                ...
                statement.setInt(8, geoposition.x());
                statement.setInt(9, geoposition.y());
